function plotFinalErrorForVars(vars, p)
    ...
    for i=1:p.size_vars
        legendInfo{i} = vars(i).observer_type+ " sensors. $N=$"+ length(vars(i).sensors) + ", $\kappa=100$" + ".";
        switch vars(i).observer_type
            case "Lagrangian"
                % legendInfo{i} = vars(i).observer_type+ " sensors. $N=$"+ length(vars(i).sensors) + ", $\kappa=100$" + ", $c=$" + vars(i).amplitude + ".";
                legendInfo{i} = vars(i).observer_type+ " sensors. $N=$"+ length(vars(i).sensors) + ".";
            case "Inertia"
                legendInfo{i} = vars(i).observer_type+ " sensors. $N=$"+ length(vars(i).sensors) + ", $St=$" + vars(i).stokes_number + ".";
            case {"Nonphysical Target Sensors" "Physical Target Sensors"}
                legendInfo{i} = vars(i).observer_type+ ", algorithm " + vars(i).alg + ". $N=$"+ length(vars(i).sensors) + ", $\kappa=100$" + ".";
        end
        time_axis = p.dt:p.dt:p.dt*length(vars(i).error_aot);
        semilogy(time_axis(1:1:length(vars(i).error_aot)), vars(i).error_aot(1:1:length(vars(i).error_aot)), vars(i).marker, 'LineWidth', 2);
        if i == 1
            hold on
        end
        if p.prod
