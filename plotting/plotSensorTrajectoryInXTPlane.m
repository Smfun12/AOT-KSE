function plotSensorTrajectoryInXTPlane(p, var)
    ...
    for i=1:size(var.sensor_history, 2)
        ...
        for j=1:size(var.sensor_history, 1)
            ...
            if dx < p.Lx/2
                sensor_trajectory(end+1) = var.sensor_history(j, i);
            else
                sensor_trajectory(end+1) = NaN;
            end
        end
        plot3(sensor_trajectory, [0, p.t], 5*ones(size(sensor_trajectory)), "LineWidth",10, "Color",[colors(i, :)]);
        % plot3(sensor_trajectory, [0, p.t], 5*ones(size(sensor_trajectory)), "LineWidth",10, "Color","k");
        % plot3(sensor_trajectory, [0, p.t], 5*ones(size(sensor_trajectory)), "LineWidth",10);
    end
    xlabel('$x$');
    ylabel('$t$');
    % title('Trajectories of Lagrangian sensors in x-t plane');
    fontsize(120, "points")
    ...
    set(gcf, 'Position', [0 800 1650 825]) 
    % grid on;
end
